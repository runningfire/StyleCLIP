...
class TrainOptions:
	...
	def initialize(self):
		...
		self.parser.add_argument('--latents_train_path', type=str, required=True, help="The latents for the training")
		self.parser.add_argument('--latents_test_path', type=str, required=True, help="The latents for the validation")
		self.parser.add_argument('--train_dataset_size', default=10000, type=int, help="Will be used only if no latents are given")
		...
		self.parser.add_argument('--id_lambda', default=0.06, type=float, help='ID loss multiplier factor')
		self.parser.add_argument('--clip_lambda', default=1.0, type=float, help='CLIP loss multiplier factor')
		self.parser.add_argument('--latent_l2_lambda', default=0.3, type=float, help='Latent L2 loss multiplier factor')

		self.parser.add_argument('--stylegan_weights', default="/path/to/weights", type=str, help='Path to StyleGAN model weights')
		self.parser.add_argument('--truncation_psi', default=0.7, type=int)
		self.parser.add_argument('--stylegan_size', default=1024, type=int)
		self.parser.add_argument('--ir_se50_weights', default='/path/to/model_ir_se50.pth', type=str, help="Path to facial recognition network used in ID loss")
		self.parser.add_argument('--checkpoint_path', default=None, type=str, help='Path to StyleCLIP model checkpoint')
