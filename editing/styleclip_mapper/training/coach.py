...
class Coach:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w = w.to(self.device)
				with torch.no_grad():
					x = self.net.decoder.synthesis(w)
				w_hat = w + 0.1 * self.net.mapper(w)
				x_hat = self.net.decoder.synthesis(w_hat)
	...
	def configure_datasets(self):
		if self.opts.latents_train_path and os.path.exists(self.opts.latents_train_path):
			...
			train_latents = []
			for b in tqdm(range(self.opts.train_dataset_size // self.opts.batch_size)):
				with torch.no_grad():
					train_latents_b = self.net.decoder.mapping(train_latents_z[b: b + self.opts.batch_size], None)
					train_latents_b = self.net.decoder.mapping.w_avg.lerp(train_latents_b, self.opts.truncation_psi)
					train_latents.append(train_latents_b)
			train_latents = torch.cat(train_latents)
			torch.save(train_latents, self.opts.latents_train_path)

		if self.opts.latents_test_path and os.path.exists(self.opts.latents_test_path):
			...
			test_latents = []
			for b in tqdm(range(self.opts.test_dataset_size // self.opts.test_batch_size)):
				with torch.no_grad():
					test_latents_b = self.net.decoder.mapping(test_latents_z[b: b + self.opts.test_batch_size], None)
					test_latents_b = self.net.decoder.mapping.w_avg.lerp(test_latents_b, self.opts.truncation_psi)
					test_latents.append(test_latents_b)
			test_latents = torch.cat(test_latents)
			torch.save(test_latents, self.opts.latents_test_path)
	...
	def calc_loss(self, w, x, w_hat, x_hat):
		...
		loss = 0.0
		if self.opts.id_lambda > 0:
			loss_id, sim_improvement, _ = self.id_loss(x_hat, x, x)
